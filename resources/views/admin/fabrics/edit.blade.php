            ...
            <div class="col-md-6">
                <div class="dt-form-group">
                    ...
                    <select name="nama_kategori" id="categorySelect" class="dt-select @error('nama_kategori') is-invalid @enderror" required onchange="checkCustomCategory(this)">
                        <option value="" disabled>-- Pilih Kategori Kain --</option>
                        @foreach($categories as $cat)
                            <option value="{{ $cat->nama_kategori }}" {{ old('nama_kategori', $fabric->category->nama_kategori ?? '') == $cat->nama_kategori ? 'selected' : '' }}>
                                {{ $cat->nama_kategori }}
                            </option>
                        @endforeach
