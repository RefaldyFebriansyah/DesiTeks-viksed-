...
<div class="dt-card border-0 shadow-sm" style="border-radius: 14px; overflow: hidden;">
    ...
    <div class="p-3 bg-white border-bottom">
        <div class="d-flex flex-column flex-md-row align-items-stretch align-items-md-center justify-content-between gap-3">
            <form method="GET" action="{{ route('gudang.suppliers.index') }}" class="d-flex align-items-center gap-2 flex-grow-1" style="max-width: 520px;">
                <div class="input-group min-w-0">
                    ...
                    @if(request('search'))
                        <a href="{{ route('gudang.suppliers.index') }}" class="btn btn-light border border-start-0 text-muted" title="Reset"><i class="bi bi-x-circle-fill"></i></a>
                    @endif
                </div>
            </form>

            <a href="{{ route('gudang.suppliers.create') }}" class="dt-btn dt-btn-primary flex-shrink-0 ms-md-auto">
