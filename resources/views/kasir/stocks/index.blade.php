@extends('layouts.app')
@section('title', 'Cek Stok Kain')
@section('page-title', 'Cek Stok Kain')

@section('content')
<div class="dt-page-header mb-3">
    <div>
        <h1 class="dt-page-title">Cek Stok Kain</h1>
        <div class="dt-breadcrumb">Kasir / Cek Stok</div>
    </div>
</div>
...
<div class="dt-card border-0 shadow-sm" style="border-radius: 14px; overflow: hidden;">
    ...
    <div class="p-3 bg-white border-bottom">
        <form method="GET" action="{{ route('kasir.stocks.index') }}" id="searchForm" class="d-flex align-items-center gap-2" style="max-width: 420px;">
            <div class="input-group min-w-0">
                ...
                <input type="text" 
                       ...
                       autocomplete="off">
                @if(request('search'))
                    <a href="{{ route('kasir.stocks.index') }}" class="btn btn-light border border-start-0 text-muted" title="Reset"><i class="bi bi-x-circle-fill"></i></a>
                @endif
            </div>
        </form>
    </div>
        ...
        <table class="dt-table mb-0 align-middle">
            ...
            @forelse($stocks as $stock)
                <tr>
                    ...
                    <td class="fw-600 text-navy" style="white-space: nowrap;">{{ $stock->fabric->nama_kain }}</td>
                    <td style="white-space: nowrap;">{{ $stock->fabric->category->nama_kategori }}</td>
                    <td class="text-end fw-600 text-navy" style="white-space: nowrap;">Rp {{ number_format($stock->fabric->harga_per_meter, 0, ',', '.') }}</td>
                    <td class="text-end fw-600 text-muted" style="white-space: nowrap;">Rp {{ number_format($stock->fabric->harga_per_rol, 0, ',', '.') }}</td>
            ...
            @empty
                <tr><td colspan="8" class="text-center py-5 text-muted">Belum ada data stok.</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
